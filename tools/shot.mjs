...
p.on('pageerror', async (e) => { console.log('pageerror', e.message.slice(0, 800)); await p.screenshot({ path: out }); process.exit(1); });
await p.goto('http://127.0.0.1:5174/' + query, { waitUntil: 'load' });
await p.waitForSelector('#menu:not([hidden])', { timeout: 240000 });
