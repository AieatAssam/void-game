...
def wheel(name, r, w, x, y, z=None, tire='ink', hub='steel'):
    ...
    rot = (math.pi / 2, 0, 0)
    return [cyl(name, r, w, loc=(x, y + w / 2, z), color=tire, rot=rot, bev=r * 0.28, seg=24),
            cyl(name + 'h', r * 0.52, w * 1.08, loc=(x, y + w * 0.54, z), color=hub, rot=rot, seg=16, bev=r * 0.1)]
...
def lolly_tree(scale=1.0, trunk='clay', leaf=('forest', 'sage'), seed=0):
    ...
    for i in range(5):
        a = i * 2 * math.pi / 5 + rnd.random()
        p.append(sphere(f'c{i+1}', (0.55 + rnd.random() * 0.2) * s,
                        loc=(math.cos(a) * 0.75 * s, math.sin(a) * 0.75 * s, (1.9 + rnd.random() * 0.8) * s),
