...
def build():
    p = []
    for i in range(3):
        p.append(box(f'seat{i}', (1.6, 0.13, 0.06), loc=(0, -0.05 + i * 0.15, 0.45), color='clay', bev=0.02))
    for i in range(2):
        p.append(box(f'back{i}', (1.6, 0.05, 0.12), loc=(0, 0.33, 0.65 + i * 0.17), color='clay', bev=0.02, rot=(0.2, 0, 0)))
    for x in (-0.65, 0.65):
