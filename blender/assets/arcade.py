...
def build():
    ...
    p = [box('walls', (D, W, H), loc=(0, 0, H / 2), color='navy', bev=0.2),
         ...
         box('roof', (D - 0.3, W - 0.3, 0.1), loc=(0, 0, H + 0.12), color='police', bev=0.03),
         torus('logo', 2.0, 0.16, loc=(0.8, 0, H + 0.25), color='hot_pink', seg=40, rseg=8),
         torus('logo2', 1.4, 0.12, loc=(0.8, 0, H + 0.25), color='siren_blue', seg=40, rseg=8),
    ...
    for s in (-1, 1):  # neon roof-edge tubes: the top-down silhouette at night
        p.append(box(f'rx{s}', (D + 0.3, 0.1, 0.1), loc=(0, s * (W / 2 + 0.1), H + 0.2), color='lilac', bev=0.03, seg=1))
        p.append(box(f'ry{s}', (0.1, W + 0.3, 0.1), loc=(s * (D / 2 + 0.1), 0, H + 0.2), color='lilac', bev=0.03, seg=1))
    for k in range(14):
    ...
    p += [box('awning', (1.4, 4.0, 0.12), loc=(D / 2 + 0.7, 0, 3.0), color='gloss_black', bev=0.04, rot=(0, 0.25, 0)),
          ...
          lathe('star', [(0.0, 0.0), (0.9, 0.0), (0.0, 0.3)], loc=(-2.4, -2.8, H + 0.2), color='gold', seg=5),
          sphere('orb', 0.5, loc=(-2.4, -2.8, H + 1.1), color='hot_pink', seg=20),
