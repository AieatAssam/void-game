...
def build():
    p = [sphere('body', 0.18, loc=(0, 0, 0.3), color='wood', seg=20, scale=(2.4, 1, 1)),
         sphere('head', 0.15, loc=(0.42, 0, 0.44), color='wood', seg=20),
         sphere('snout', 0.08, loc=(0.56, 0, 0.41), color='sand', seg=12, scale=(1.3, 1, 0.9)),
         sphere('nose', 0.035, loc=(0.65, 0, 0.43), color='ink', seg=8),
         cyl('tail', 0.03, 0.22, loc=(-0.4, 0, 0.35), color='wood', rot=(0, -0.9, 0), seg=8, bev=0),
    ...
    for x in (0.28, -0.28):
        for s in (-1, 1):
            p.append(cyl(f'leg{x}{s}', 0.045, 0.2, loc=(x, s * 0.1, 0.1), color='wood', seg=10, bev=0.015))
